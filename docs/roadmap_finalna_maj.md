...
| T2.11 | ✅ (U) | **Walidacja pola "Kod" dla typów i szablonów** — auto-slugifikacja (spacje→`_`, usunięcie diakrytyków) przy `onChange`; regex `^[a-z][a-z0-9_]{0,29}$` blokuje submit; tekst pomocniczy + komunikat błędu pod polem; dotyczy `AdminEventTypes.tsx` (dodaj) i `AdminMessageTemplates.tsx` (dodaj + edytuj). | `AdminEventTypes.tsx`, `AdminMessageTemplates.tsx` | 0.25 dnia |
| T2.12 | ✅ (U) | **Biblioteka ikon dla typów zdarzeń** — pole `icon_key VARCHAR(50)` w `EventType` + migracja Alembic z seedem 3 typów; 8-elementowy słownik ikon Lucide w `eventTypeIcons.ts`; wizualny picker w formularzach add/edit AdminEventTypes; kolumna Ikona w tabeli; EventCard i AdminDashboard używają `icon_key` z API zamiast hardkodowanej mapy. | `models/event_type.py`, `schemas/event_type.py`, `routers/event_types.py`, migracja, `eventTypeIcons.ts`, `useEventTypes.ts`, `AdminEventTypes.tsx`, `EventCard.tsx`, `AdminDashboard.tsx` | 0.5 dnia |
...
| T2.11 | 2026-05-05 | Auto-slugifikacja pola "Kod" (spacje→_, diakrytyki usunięte) przy onChange + walidacja regex w obu formularzach; tekst pomocniczy i komunikat błędu pod polem; submit zablokowany przy błędnym kodzie; build ✅ 5.01s | `frontend/src/pages/AdminEventTypes.tsx`, `frontend/src/pages/AdminMessageTemplates.tsx` |
| T2.12 | 2026-05-05 | Biblioteka 8 ikon Lucide w eventTypeIcons.ts + resolveIcon(); icon_key VARCHAR(50) w modelu EventType + migracja z seedem; wizualny picker (siatka przycisków) w formularzach add/edit AdminEventTypes + kolumna Ikona w tabeli; EventCard i AdminDashboard używają icon_key z API; backend import-check PASS, frontend build ✅ 5.24s, migracja HEAD | `backend/app/models/event_type.py`, `backend/app/schemas/event_type.py`, `backend/app/routers/event_types.py`, `backend/alembic/versions/20260505_add_icon_key_to_event_types.py`, `frontend/src/lib/eventTypeIcons.ts`, `frontend/src/hooks/useEventTypes.ts`, `frontend/src/pages/AdminEventTypes.tsx`, `frontend/src/components/EventCard.tsx`, `frontend/src/pages/AdminDashboard.tsx` |
| Ad-Hoc UI-panel | 2026-05-05 | Poprawa czytelności panelu Aktywnych Zdarzeń: sidebar poszerzony lg:400→460px / xl:450→520px; daty rozpoczęcia i zakończenia planowego wyłączenia rozbite na osobne wiersze w grid-cols-[1rem_1fr] (CalendarClock + Clock); CardContent space-y-2→space-y-2.5; build ✅ 5.24s | `frontend/src/pages/Index.tsx`, `frontend/src/components/EventCard.tsx` |
